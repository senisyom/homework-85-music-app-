...
export const createAlbum = createAsyncThunk<
  ...
  AlbumMutation,
  { state: RootState }
>("albums/create", async (albumMutation, { getState }) => {
  ...
  if (user) {
    ...
    await axiosApi.post<IAlbum>(`/albums`, formData, {
      headers: { Authorization: `Bearer ${user.token}` },
    });
  }
});

export const publishAlbum = createAsyncThunk<IAlbum, string>(
  "albums/publish",
  async (id) => {
    const { data: album } = await axiosApi.patch<IAlbum>(
...
export const deleteAlbum = createAsyncThunk<IAlbum, string>(
  "albums/delete",
  async (id) => {
